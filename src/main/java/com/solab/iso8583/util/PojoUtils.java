...
public class PojoUtils {
  ...
  /**
	...
	/**
	 * Convert the given Date into an instance of the given target class.
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static <T> T convertDateToTargetClass(Date dt, Class<T> targetClass, TimeZone timeZone) {
		assert null != dt: "Number must not be null";
		assert null != targetClass : "Target class must not be null";
		if (targetClass.isInstance(dt)) {
			return (T) dt;
		} 
		if (Calendar.class == targetClass) {
			Calendar cal = null == timeZone ? Calendar.getInstance() : Calendar.getInstance(timeZone);
			cal.setTime(dt);
			return (T) cal;
		}  
		...
		if (LocalDateTime.class == targetClass) {
			...
			LocalDate ld = LocalDateTime.ofInstant(dt.toInstant(), zoneId).toLocalDate();
			return (T) ld;
		} else if (ZonedDateTime.class == targetClass) {
			ZonedDateTime zdt = ZonedDateTime.ofInstant(dt.toInstant(), zoneId);
			return (T) zdt;
		} else if (LocalTime.class == targetClass) {
			LocalTime lt = LocalDateTime.ofInstant(dt.toInstant(), zoneId).toLocalTime();
			return (T) lt;
		} else {
			throw new IllegalArgumentException(String.format("Could not convert date [%d] of type [%s] to target class [%s]: overflow", dt, dt.getClass().getName(), targetClass.getName()));
		}
	}
}
